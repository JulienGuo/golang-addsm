...
func ReadCertificateRequestFromMem(data []byte) (*x509.CertificateRequest, error) {
	block, _ := pem.Decode(data)
	if block == nil {
...
func ReadCertificateRequestFromPem(FileName string) (*x509.CertificateRequest, error) {
	data, err := ioutil.ReadFile(FileName)
	if err != nil {
		return nil, err
	}
...
func CreateCertificateRequestToMem(template *x509.CertificateRequest, privKey *sm.PrivateKey) ([]byte, error) {
...
func CreateCertificateRequestToPem(FileName string, template *x509.CertificateRequest,
privKey *sm.PrivateKey) (bool, error) {
	der, err := x509.CreateCertificateRequest(rand.Reader, template, privKey)
	if err != nil {
		return false, err
	}
...
func ReadCertificateFromMem(data []byte) (*x509.Certificate, error) {
	block, _ := pem.Decode(data)
	if block == nil {
		return nil, errors.New("failed to decode certificate request")
	}
...
func ReadCertificateFromPem(FileName string) (*x509.Certificate, error) {
	data, err := ioutil.ReadFile(FileName)
	if err != nil {
		return nil, err
	}
...
func CreateCertificateToMem(template, parent *x509.Certificate, pubKey *sm.PublicKey, privKey *sm.PrivateKey) ([]byte, error) {
...
func CreateCertificateToPem(FileName string, template, parent *x509.Certificate, pubKey *sm.PublicKey, privKey *sm.PrivateKey) (bool, error) {
	der, err := x509.CreateCertificate(rand.Reader, template, parent, pubKey, privKey)
	if err != nil {
		return false, err
	}
...
func ReadKeyFromMem(data []byte, pwd []byte) (sm.SM4Key, error) {
	block, _ := pem.Decode(data)
...
func ReadKeyFromPem(FileName string, pwd []byte) (sm.SM4Key, error) {
	data, err := ioutil.ReadFile(FileName)
	if err != nil {
		return nil, err
	}
...
func WriteKeytoMem(key sm.SM4Key, pwd []byte) ([]byte, error) {
	if pwd != nil {
...
func WriteKeyToPem(FileName string, key sm.SM4Key, pwd []byte) (bool, error) {
	var block *pem.Block
